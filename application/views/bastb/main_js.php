<script>
    ...
    function SubmitTambah(){
        if(Validasi() != false){
            ...
            $.ajax({
                type : "POST",
                url : "<?= base_url('bastb/simpan'); ?>",
                data : iData,
                success: function(r){
                    var response = JSON.parse(r);
                    console.log(response);
                    if(response['status'] === false){
                        FormMessage("Modul Dokumen Berita Acara Serah Terima Barang",response['message'],'error');
                    }else{
                        FormMessage('Modul Dokumen Berita Acara Serah Terima Barang', response['message']);
                        setTimeout(function(){
